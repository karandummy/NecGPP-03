  ...
  if (role === 'Student' && evaluation.passed) {
    ...
    try {
      ...
      if (passedSets.passed_count === totalSets.total) {
        ...
        if(level===1){
          await conn.execute(
            `update students set lev_1_completed=lev_1_completed+1 where student_id=?`,[userId]
          );
        }
        else if(level===2){
          await conn.execute(
            `update students set lev_2_completed=lev_2_completed+1 where student_id=?`,[userId]
          );
          await conn.execute(
            `update students set topics_completed=topics_completed+1 where student_id=?`,[userId]
          );
        }
      }
