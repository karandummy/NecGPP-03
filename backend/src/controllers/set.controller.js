...
export const getSetsByLevel = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const [allSets] = await conn.execute(
      `SELECT set_id, display_order,total_marks,threshold_percentage,negative_marking
       FROM practice_sets
    ...
    if (role !== 'Student') {
      return successResponse(res, {
        ...
        level,
        allSets:allSets,
        total_no_of_sets: totalSets,
        accessible_sets_ids: allSetIds,
        all_sets_ids: allSetIds
      });
    }
    ...
    return successResponse(res, {
      ...
      allSets:allSets,
      total_no_of_sets: totalSets,
      accessible_sets_ids: Array.from(accessibleSetIds),
      all_sets_ids: allSetIds
    });
...
export const deleteSetWithBackup = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    return successResponse(res, null, "Set deleted with backup");
  } catch (err) {
    await conn.rollback();
