...
export const deleteSubjectWithBackup = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    if (!canDelete) {
      conn.release();
      return next(new AppError("You do not have permission to delete this subject", 403));
    }

    // // Generate both Excel files FIRST using mock response
    // const contentBuffer = await generateExcelBuffer(exportSubjectAsExcel, req, "content");
    // const attemptsBuffer = await generateExcelBuffer(exportSubjectAsExcel, req, "attempts");

    // // Now set headers and stream ZIP
    ...
    // archive.append(contentBuffer, { name: `subject_${subjectId}_content.xlsx` });
    // archive.append(attemptsBuffer, { name: `subject_${subjectId}_attempts.xlsx` });
    
    // await archive.finalize();

    // Finally delete the subject
    await conn.execute(`DELETE FROM subjects WHERE subject_id = ?`, [subjectId]);

    conn.release();
    // Response already sent via archive.pipe(res)

    return successResponse(res, null, "Subject deleted with backup");
    ...
    next(
      err instanceof AppError
        ? err
        : new AppError("Failed to delete subject with backup", 500)
    );
  }
});