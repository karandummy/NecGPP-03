...
export const reorderTopics = catchAsync(async (req, res, next) => {
  ...
  const subjectId = req.subjectId;
  const userId = req.user.userId;
  console.log(ordered_topic_ids,subjectId,userId)
  ...
  try {
    ...
    // Use UPDATE ... CASE to set display_order for existing topics
    const inPlaceholders = ordered_topic_ids.map(() => '?').join(',');
    const caseClauses = ordered_topic_ids.map(() => 'WHEN ? THEN ?').join(' ');
    const params = [];
    ordered_topic_ids.forEach((id, i) => {
      params.push(id, i + 1);
    });
    // Append updated_by and subjectId, then the IN list params
    params.push(userId, subjectId, ...ordered_topic_ids);

    await conn.execute(
      `UPDATE topics
       SET display_order = CASE topic_id ${caseClauses} ELSE display_order END,
           updated_by = ?,
           updated_at = NOW()
       WHERE subject_id = ? AND topic_id IN (${inPlaceholders})`,
      params
    );
    ...
    await conn.rollback();
    next(err instanceof AppError ? err : new AppError(`Failed to reorder topics: ${err.message}`, 500));
  } finally {
    conn.release();
  }
});
