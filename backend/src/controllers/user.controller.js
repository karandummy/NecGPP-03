...
export const getMyBasicProfile = catchAsync(async (req, res, next) => {
    ...
    case 'Student':
      query = `
        SELECT 
          ...
          COALESCE(s.lev_1_completed, 0) AS no_of_lev1_completed,
          COALESCE(s.lev_2_completed, 0) AS no_of_lev2_completed,
          COALESCE(s.topics_completed, 0) AS no_of_topics_completed,
          d.dept_code
        FROM users u
  ...
  if (role === 'Student') {
    response.dashboard = {
      ...
      no_of_lev1_completed: Number(data.no_of_lev1_completed),
      no_of_lev2_completed: Number(data.no_of_lev2_completed),
      no_of_topics_completed: Number(data.no_of_topics_completed)
    };
  }
...
`;
  ...
  if (role === 'Student') {
    Object.assign(profile, {
      ...
      no_of_lev1_completed: Number(u.no_of_lev1_completed),
      no_of_lev2_completed: Number(u.no_of_lev2_completed),
      no_of_topics_completed: Number(u.no_of_topics_completed),
