...
const FIXED_COL_WIDTH = 24; // ≈ 6 cm

const setFixedColumnWidth = (data) => {
  return data[0].map(() => ({ wch: FIXED_COL_WIDTH }));
};



// Helper function to generate Excel buffer
// src/utils/backup.util.js
...
export const generateExcelBuffer = async (exportFunction, req, exportType) => {
  const mockRes = {
    ...
    // In case some code calls res.end()
    end: function (data) {
      if (data) this.body = data;
    },
  };

  // This next will THROW on any error instead of silently exiting
  const throwingNext = (err) => {
    if (err) {
      throw err instanceof Error ? err : new Error(err.message || "Export failed");
    }
  };

  try {
    const mockReq = {
      ...req, // copies subjectId, user, etc.
      query: { ...req.query, export_type: exportType },
      res: mockRes, // in case function uses req.res
    };

    // Call the export function — it will now throw on next(error)
    await exportFunction(mockReq, mockRes, throwingNext);

    if (mockRes.body && Buffer.isBuffer(mockRes.body)) {
      return mockRes.body;
    } else {
      throw new Error(
        mockRes.body
          ? "Data sent but not a Buffer"
          : "res.send was never called — likely early exit"
      );
    }
  } catch (err) {
    // Now you will see the REAL error in console
    console.error(`Failed to generate ${exportType} Excel:`, err);
    throw err; // re-throw so delete controller catches it
  }
};
...
export const exportSetAsExcel = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    if (export_type === 'content') {
      ...
      ws1['!cols'] = setFixedColumnWidth(setInfoData);
      ...
      ws2['!cols'] = setFixedColumnWidth(questionsData);
      ...
      ws2['!cols'] = setFixedColumnWidth(attemptsData);
...
export const exportTopicAsExcel = catchAsync(async (req, res, next) => {
  const topicId = req.topicId; // from attachTopic middleware
  const { export_type } = req.query; 
  const conn = await pool.getConnection();
  try {
    // Get topic details
    const [topicDetails] = await conn.query(
    ...
    if (export_type === 'content') {
      ...
      ws1['!cols'] = setFixedColumnWidth(topicInfoData);
      ...
      ws2['!cols'] = setFixedColumnWidth(setsData);
      ...
      ws3['!cols'] = setFixedColumnWidth(questionsData);
      ...
      ws3['!cols'] = setFixedColumnWidth(attemptsData);
...
export const exportSubjectAsExcel = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    if (export_type === 'content') {
      ...
      ws1['!cols'] = setFixedColumnWidth(subjectInfoData);
      ...
      ws2['!cols'] = setFixedColumnWidth(topicsData);
