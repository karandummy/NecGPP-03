// src/components/subjects/ExportModal.jsx
import { useState } from 'react';
import { Download, FileSpreadsheet } from 'lucide-react';
import Modal from '@components/common/Modal';
import Button from '@components/common/Button';

const ExportModal = ({ isOpen, onClose, onExport, subject, loading }) => {
  const [exportType, setExportType] = useState('content');

  const handleExport = async () => {
    await onExport(subject.subject_id, exportType);
  };
  ...
  return (
    <Modal 
      isOpen={isOpen} 
      onClose={onClose} 
      title={`Export Data - ${subject?.subject_name}`} 
      size="sm"
    >
          ...
          <div className="space-y-3">
            <label className="flex items-start gap-3 p-4 border-2 rounded-lg cursor-pointer transition-all hover:border-primary-300 hover:bg-primary-50">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-600">
                  Export all subjects, topics, sets, and questions
                </p>
              </div>
            </label>
            ...
            <label className="flex items-start gap-3 p-4 border-2 rounded-lg cursor-pointer transition-all hover:border-primary-300 hover:bg-primary-50">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-600">
                  Export all student practice attempts and scores
                </p>
              </div>
            </label>
          </div>
